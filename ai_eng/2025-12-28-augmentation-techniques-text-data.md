...
In this qubit note we will be looking into ways to augment text data. Augmenting text data can be more challenging than when working with image data, where simple transformations such as rotation or flipping can create valid new samples.  When augmenting text data we need to maintian semantic integrity and coherence. Just like with image data augmentation, our goal is
to increase the dataset size and diversity, adderess data imbalance and data bias so that we improve model robustness.
...
- Synonym replacement; that is replace words with their synonyms. This technique involves replacing words in the original text with their synonyms. We can use <a href="https://en.wikipedia.org/wiki/WordNet">WordNet</a> to find synonyms or use T5.
