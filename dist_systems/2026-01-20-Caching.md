...
Caching is a specific data replication technique that offers best efforτ guarantees [2].
Specifically, ιn this note, we will briefly discuss the following approches to caching:
...
of the data closer to where actually the data is accessed (Note that we can also use  <a href="2026-04-28-Negative-Caching.md">Negative Caching</a>). 
Thus, apart from the main storage, when using caching, we maintian one or more copies of the data to one or more locations [1]. With web applications,
when we cache data we don't copy the whole data but rather a small portion of it. Caching unavoidably brings up the
issue of data consistency i.e. how up to date the cached data is. We will discuss this  in <a href="2026-01-26-Cache-Replacement.md">Cache Replacement</a>.

---
**Remark: Caching is difficult**

The main challenge when we discuss caching is the cache  invalidation i.e ensuring cached data updates when the underlying data changes. 
Phil Karlton famously called it one of the two hard problems in computer science [3]. 
For example, Meta's engineering team spent years improving their TAO cache consistency from six nines to ten nines meaning from 99.9999% to 99.99999999% [3]. 

The difficulty with cache invalidation arises form the nature of modern systems; data gets mutated both when filling the cache on reads and when processing invalidations on writes.
When a cache tries to fill a value while an invalidation is in flight, they can arrive out of order, leaving stale data indefinitely [3].
...
3. _Performance Engineering in Practice_, Manning Publications
